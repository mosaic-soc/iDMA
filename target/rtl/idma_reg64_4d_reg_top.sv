...
module idma_reg64_4d_reg_top #(
    ...
    // Field: idma_reg.conf.deadlock_free
    always_comb begin
        automatic logic [0:0] next_c;
        automatic logic load_next_c;
        next_c = field_storage.conf.deadlock_free.value;
        load_next_c = '0;
        if(decoded_reg_strb.conf && decoded_req_is_wr) begin // SW write
            next_c = (field_storage.conf.deadlock_free.value & ~decoded_wr_biten[19:19]) | (decoded_wr_data[19:19] & decoded_wr_biten[19:19]);
            load_next_c = '1;
        end
        field_combo.conf.deadlock_free.next = next_c;
        field_combo.conf.deadlock_free.load_next = load_next_c;
    end
    always_ff @(posedge clk or negedge arst_n) begin
        if(~arst_n) begin
            field_storage.conf.deadlock_free.value <= 1'h0;
        end else begin
            if(field_combo.conf.deadlock_free.load_next) begin
                field_storage.conf.deadlock_free.value <= field_combo.conf.deadlock_free.next;
            end
        end
    end
    assign hwif_out.conf.deadlock_free.value = field_storage.conf.deadlock_free.value;
    ...
    always_comb begin
        ...
        if(rd_mux_addr == 9'h0) begin
            ...
            readback_data_var[19] = field_storage.conf.deadlock_free.value;
        end
        for(int i0=0; i0<16; i0++) begin
